...
def plot_predictions_curves(data,T, T_orig,P_row, P_col, masked_tokens, masked_pos, logits_lm, dev_logits_lm, logits_lm_with_dummy, patch_size,
    ...
    if tmp_size > 2:
        ...
        if n_cols*n_rows<tmp_size: n_rows+=1
...
def create_dataloaders_ours(path_file, experiment_name, use_first_n_frames, batch_size_segments, validation_split, regularly_sampled, downsample_points,args, verbose=False):
    '''
        Zitian: note to myself: check whether the scaling is suitable for our data
    '''
    Data = [
        np.array(pd.read_csv(f'{path_file}/{i}.tsv.gz', sep='\t')) # c x d
        for i in range(1, 17)
    ]
    Data = np.stack(Data, axis=1)[:, :, :, None] # c x T x d x 1
    ...
    return dataloaders, scaling_factor, Train_Data_indices, Val_Data_indices
...
def create_dataloaders_toydata(path_file, experiment_name, use_first_n_frames, batch_size_segments, validation_split, regularly_sampled, downsample_points,args, verbose=False):
    ...
    Data = Data_dict['Data_orig'][:,:use_first_n_frames,:, None] # n x T x d x 1
