...
class BERT_Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # batch 0 shape torch.Size([10, 16, 100, 1])
        # batch 1 shape torch.Size([10, 16])
        # batch 2 shape torch.Size([10])
        # batch 3 shape torch.Size([10])
        ...
        # T shape torch.Size([516])
        # segm_frames shape torch.Size([10, 516, 100])
        # masked_pos shape torch.Size([10, 16])
        # masked_tokens shape torch.Size([10, 16, 100, 1])
...
#         mask : (B(batch_size) x S(seq_len))
    ...
    def inference(self, dataloader):
        ...
        for batch in tqdm(dataloader):
            ...
            '''
                ...
                masked_tokens == data
            '''
            # print(T.shape, y.shape, masked_pos.shape, masked_tokens.shape, data.shape)
            # torch.Size([516]) torch.Size([10, 516, 100, 1]) torch.Size([10, 16]) torch.Size([10, 16, 100, 1]) torch.Size([10, 16, 100, 1]) 
            btz = y.shape[0]
            T = torch.stack([T] * btz)
            Ts.append(T.detach().cpu().numpy())
            ys.append(y[..., 0].detach().cpu().numpy())
    ...
    def inference_step(self, batch):
        ...
        data = data.to(device)
        latentVar = self.model.encoder(segm_frames.type_as(data))
        # print('la var device', latentVar.device)
