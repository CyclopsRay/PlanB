...
setwd("G:/My Drive/Singh Lab/Code/python/aim2/cao2019")
#base_path <- getwd()
#setwd(base_path)
...
cell_metadata <- read.csv('cell_annotate.csv')
gene_annotation <- read.csv('gene_annotate.csv')

print('done with cell and gene metadata')

cds <- readRDS('gene_count_cleaned_sampled_100k.RDS')
...
mouse_gtf <- fread("gencode.vM26.annotation.gtf")
...
sObj <- NormalizeData(sObj, normalization.method='RC', scale.factor=scale_factor)

### Select all time points except for 13.5 (last time point)
###   and save into new Seurat object
sObj_cells_at_tp_not_5 <- subset(x = sObj, subset = time_pt != 13.5)
...
###   except last
sObj_var_genes <- FindVariableFeatures(sObj_cells_at_tp_not_5, selection.method = "vst", nfeatures = 2000)
var_genes <- VariableFeatures(sObj_var_genes)
sObj_var_genes <- sObj[var_genes,]


### Write each time point to .mtx file
